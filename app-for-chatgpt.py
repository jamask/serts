...
def get_squid_logs():
    """
    Read Squid access.log through the privileged helper.

    Helper output format:

        LOG_SIZE=166M
        ACCESS_LOG_EXISTS=yes
        ---LOG---
        <squid access.log lines>
    """
    ...
    try:
        ...
        if proc.returncode != 0:
            result["error"] = (
                stderr
                or output.strip()
                or f"Helper exited with code {proc.returncode}"
            )
            return result
        ...
        for i, line in enumerate(lines):
            ...
            elif line.startswith("ACCESS_LOG_EXISTS="):

                exists = line.split("=", 1)[1].strip().lower()

                if exists != "yes":
                    result["error"] = "Squid access.log does not exist."
        ...
        if log_start is None:
            result["error"] = (
                result["error"]
                or "Invalid helper output: ---LOG--- marker not found."
            )
            return result
        ...
        result["error"] = "Timeout while reading Squid access.log."
...
def parse_squid_log_line(line):
    """
    Parse standard Squid access.log format.

    Example:

    1788244058.003 0 10.218.14.44 TCP_DENIED/403 3190
    CONNECT clients1.google.com:443 - HIER_NONE/- text/html
    """
    ...
    try:
        ...
        content_type = parts[8] if len(parts) > 8 else ""

        # ----------------------------------------------------
        # Status classification
        # ----------------------------------------------------

        if status.startswith("TCP_DENIED/"):

            result_status = "DENIED"

        elif status.startswith("NONE_NONE/"):
            ...
            try:
                status_code = int(status.split("/", 1)[1])
...
def logs():
    ...
    for entry in all_entries:
        ...
        if filter_value == "allowed":
            ...
            if entry["result_status"] != "ALLOWED":
                continue

        elif filter_value == "denied":

            if entry["result_status"] != "DENIED":
                continue
...
def get_active_proxy_clients():
    ...
    for line in result.stdout.splitlines():
        ...
        if ":3128" not in local:
            continue

        client_ip = remote.rsplit(":", 1)[0]
...
def api_active_clients():
    ...
    active_proxy_clients = get_active_proxy_clients()

    return {
        "count": len(active_proxy_clients),
        "clients": active_proxy_clients
    }
...
def index():
    ...
    return render_template(
        ...
        active_proxy_clients=active_proxy_clients,
        websites=websites,
